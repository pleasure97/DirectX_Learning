...
bool GlassShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	// Setup the description of the glass dynmaic constant buffer that is in the pixel shader. 
...
void GlassShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFilename)
{
	...
	MessageBox(hwnd, L"Error compiling shader. Check shader-error.txt from message.", shaderFilename, MB_OK); 
	...
	ID3D11ShaderResourceView* colorTexture, ID3D11ShaderResourceView* normalTexture, ID3D11ShaderResourceView* refractionTexture, float refractionScale)
{
	...
	// Set the 3 shader texture resources in the pixel shader. 
