...
ScrollShaderClass::ScrollShaderClass(const ScrollShaderClass&)
{
}
...
bool ScrollShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
    ...
    error = wcscpy_s(vsFilename, 128, L"../DirectXEngine/scroll.vs");
    if (error != 0)
    {
        return false;
    }
    ...
    error = wcscpy_s(psFilename, 128, L"../DirectXEngine/scroll.ps");
    if (error != 0)
    {
        return false;
    }
...
bool ScrollShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, 
...
bool ScrollShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, 
    XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, 
    ID3D11ShaderResourceView* texture, float translation)
{
    HRESULT result;
