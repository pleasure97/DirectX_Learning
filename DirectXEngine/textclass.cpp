...
bool TextClass::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, 
							...
							int maxLength, FontClass* Font, char* text,
							...
							float red, float green, float blue)
{
	...
	// Use the font class to bulid the vertex array from the sentence text and sentence draw location
...
bool TextClass::InitializeBuffers(ID3D11Device* device, ID3D11DeviceContext* deviceContext, 
									FontClass* Font, char* text, int positionX, int positionY, 
									float red, float green, float blue)
{
	VertexType* vertices; 
	...
	result = UpdateText(deviceContext, Font, text, positionX, positionX, red, green, blue); 
	if (!result) { return false; }
