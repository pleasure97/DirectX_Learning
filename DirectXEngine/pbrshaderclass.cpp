...
PbrShaderClass::PbrShaderClass(const PbrShaderClass&)
{
}
...
bool PbrShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
    ...
    error = wcscpy_s(vsFilename, 128, L"../DirectXEngine/pbr.vs");
    if (error != 0)
    {
        return false;
    }
    ...
    error = wcscpy_s(psFilename, 128, L"../DirectXEngine/pbr.ps");
    if (error != 0)
    {
        return false;
    }
