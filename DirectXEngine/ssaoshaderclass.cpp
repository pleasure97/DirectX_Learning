...
bool SsaoShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
    ...
    error = wcscpy_s(vsFilename, 128, L"../DirectXEngine/ssao.vs");
    if (error != 0)
    {
        return false;
    }
    ...
    error = wcscpy_s(psFilename, 128, L"../DirectXEngine/ssao.ps");
    if (error != 0)
    {
        return false;
    }
...
bool SsaoShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, 
    ...
    ID3D11ShaderResourceView* positionTexture, ID3D11ShaderResourceView* normalTexture, ID3D11ShaderResourceView* randomTexture, 
    float screenWidth, float screenHeight, 
    float randomTextureSize, float sampleRadius, 
    ...
    float ssaoScale, float ssaoBias, float ssaoIntensity)
{
    ...
    // Get a pointer to the data into the ssao constant buffer.
