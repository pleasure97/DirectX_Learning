...
bool ParticleSystemClass::InitializeParticleSystem()
{
	...
	// Set the random deviation of where the particles can be loacted when emitted.
...
bool ParticleSystemClass::InitializeBuffers(ID3D11Device* device)
{
	...
	// Set the maximum number of indices in the vertex array.
...
void ParticleSystemClass::EmitParticles(float frameTime)
{
	...
	if ((emitParticle == true) && (m_currentParticleCount < (m_maxParticles - 1)))
	{
		...
		// Now insert into the particle array in the correct depth order.
...
void ParticleSystemClass::UpdateParticles(float frameTime)
{
	...
	{
		m_particleList[i].positionY -= m_particleList[i].velocity * frameTime; 
	}
...
void ParticleSystemClass::KillParticles()
{
	...
	for (i = 0; i < m_maxParticles; i++)
	{
		if ((m_particleList[i].active == true) && (m_particleList[i].positionY < -3.f))
		{
			...
			// Now shift all the live partices back up the array to erase the destroyed particle and keep the array sorted correctly.
...
bool ParticleSystemClass::UpdateBuffers(ID3D11DeviceContext* deviceContext)
{
	...
	// Now build the vertex array from the particle list array. Each particle is a quad made out of 2 triangles.
	index = 0; 
	...
	for (i = 0; i < m_currentParticleCount; i++)
	{
		...
		m_vertices[index].color = XMFLOAT4(m_particleList[i].red, m_particleList[i].green, m_particleList[i].blue, 1.f); 
		index++; 
